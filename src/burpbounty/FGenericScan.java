...
public class FGenericScan {
    ...
    public FGenericScan(BurpBountyExtension bbe, IBurpExtenderCallbacks callbacks, CollaboratorData burpCollaboratorData, String filename, JsonArray allprofiles, Tags tagui) {
    ...
    public List<IScanIssue> runAScan(IHttpRequestResponse baseRequestResponse, IScannerInsertionPoint insertionPoint, JsonArray activeprofiles, String bchost) {
        ...
        for (int i = 0; i < activeprofiles.size(); i++) {
            ...
            for (String payload : payloads) {
                ...
                if (matchtype == 5) {//no cambiar a switch
                    ...
                    try {
                        if (matches != null) {
                            ...
                            if(issues.size()>0)
                            {
                                this.tagui.payloads.add(payload);
                                this.tagui.setBackgroundColor(new Color(0xFF,0,0));
                                //this.tagui.setOneRowBackgroundColor(0,new Color(0xFF,0,0));
                                issues.clear();
                            }
                            startIndex = this.tagui.getRowCount();
                            //this.tagui.add(helpers.analyzeRequest(requestResponse).getUrl(), String.valueOf(responseCode), requestResponse);
                           continue;
                        }
                    } catch (Exception e) {
                ...
                if(issues.size()>0)
                {
                    //this.tagui.setLightCows(0,payload);
                    this.tagui.payloads.add(payload);
                    //this.tagui.setOneRowBackgroundColor(0,new Color(0xFF,0,0));
                    this.tagui.setBackgroundColor(new Color(0xFF,0,0));
                    issues.clear();
